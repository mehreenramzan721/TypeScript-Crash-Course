...
class Person implements PersonInterface {
  // if we just write these two lines then we will get an error because they need to be initialized inside a constructor
  id: number;
  name: string;
...
const e1 = new Employee(101, "ahs", "as");
...
// let stringArray = getArray(["brad", "gad"]);
// numArray.push("da")// we wil not get an error because we specified the type to any so in this case
// // we will use generic
...
let stringArray = getArray<string>(["brad", "gad"]);
// numArray.push("da"); // get an error because now it is only num
